...
def add_slurs(note_rhythm_list, slur_chance=0.7):
    """
    Returns a list of slur markers (None, '(', ')') per note, to be inserted after the rhythm.
    Slurs are only for runs of two or more consecutive notes (not rests) with the same rhythm,
    never spanning rests or rhythm changes. Each run gets a slur with the given probability.
    """
    slur_marks = [None] * len(note_rhythm_list)
    i = 0
    while i < len(note_rhythm_list):
        # Only begin at a note (not rest)
        if note_rhythm_list[i][0] != 'rest':
            rhythm = note_rhythm_list[i][1]
            # Find run length
            j = i
            while (j < len(note_rhythm_list)
                   and note_rhythm_list[j][0] != 'rest'
                   and note_rhythm_list[j][1] == rhythm):
                j += 1
            run_len = j - i
            if run_len >= 2 and random.random() < slur_chance:
                slur_marks[i] = '('
                slur_marks[j-1] = ')'
            # Move to next run
            i = j
...
def format_note_sequence_with_slurs(note_rhythm_list, articulation_list, slur_marks, inter_note_code=None):
    output = []
    for i, current in enumerate(note_rhythm_list):
        articulation = articulation_list[i] if articulation_list else ""
        slur = slur_marks[i] if slur_marks else None
        ...
        if current[0] == 'rest':
            ...
            pitch = midi_to_lilypond(current[0])
            rhythm = str(current[1])
            # Insert slur parens after rhythm but before articulation/etc
            note_core = f"{pitch}{rhythm}"
            if slur == '(':
                note_core += '('
            elif slur == ')':
                note_core += ')'
            note_string = f"{note_core}{articulation}"
            print(note_string, end=' ')  # Print melody note to terminal
...
def main():
    ...
    try:
        ...
        # ---- SLUR LOGIC ADDED HERE ----
        slur_marks = add_slurs(combined, slur_chance=1)  # Set your probability here
        lilypond_notes = format_note_sequence_with_slurs(combined, articulation_sequence, slur_marks, inter_note_code)
        # --------------------------------
